...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("out/conical_journal/studies/jfo-checkpoint-evidence"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        _style()
        checkpoints, rupture_pressure_pa = load_checkpoints(case)
        write_metrics(checkpoints, stage)
        plot_speed_sweep(checkpoints, stage)
        plot_final_fields(checkpoints[-1], rupture_pressure_pa, stage)
        plot_fill_severity(checkpoints[-1], stage)
        animate_checkpoints(checkpoints, stage)
        upstream = [
            case / "system/blockMeshDict",
            case / "constant/jfoProperties",
            case / "checkpoint_manifest.csv",
        ]
        for _rpm, time_name in _checkpoint_specs(case):
            upstream.extend(
                case / time_name / name
                for name in ("p", "thetaFill", "surfaceMetric")
            )
        publish_generation(
            stage,
            target,
            stage="study",
            operation="jfo-checkpoint-evidence",
            status="RENDERED",
            resolved_inputs={"case": case, "checkpoints": _checkpoint_specs(case)},
            input_units={
                "pressure": "Pa in source; MPa gauge in plots",
                "speed": "rpm",
                "surface_metric": "m^2 per radian-axial cell",
            },
        ...
        if stage.exists():
            shutil.rmtree(stage)
    print(f"wrote JFO figures and animation to {target}")
    return 0
