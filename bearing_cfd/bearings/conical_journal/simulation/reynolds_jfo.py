...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("out/conical_journal/simulation/reynolds-jfo"),
    )
    ...
    try:
        ...
        for rpm in args.rpm:
            ...
            summary = write_results(stage / _stage_name(rpm), inputs, grid, state)
            ramp.append(summary)
            previous_fill = state.fill_fraction
            if not summary["acceptance"]["accepted"]:
                break

        (stage / "ramp_summary.json").write_text(
            json.dumps(ramp, indent=2) + "\n", encoding="utf-8"
        )
        accepted = len(ramp) == len(args.rpm) and all(
        ...
        publish_generation(
            ...
            upstream_artifacts=(args.initial_state,) if args.initial_state else (),
            tool_versions={
