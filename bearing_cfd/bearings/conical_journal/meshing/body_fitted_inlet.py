...
from bearing_cfd.bearings.conical_journal.meshing import surface_inlet as staircase
...
from bearing_cfd.bearings.conical_journal.meshing.surface_inlet import InletSpec, load_inlet_spec
from bearing_cfd.interchange.cgns import sanitize_gmsh_cgns
...
def export_body_fitted_case(
    ...
    *,
    ...
    if minimum_fluent_orthogonal_quality is not None:
        fluent_model = fluent_legacy.build_fluent_legacy_mesh(npz_path)
        fluent_oq = fluent_legacy.orthogonal_quality(fluent_model)
        mesh = replace(
            mesh,
        ...
        independent_audit = (
            fluent_legacy.independent_centroid_orthogonal_quality_audit(
                fluent_model,
                minimum_fluent_orthogonal_quality,
            )
        )
        ...
        del fluent_model
        fluent_result = fluent_legacy.write_fluent_legacy_mesh(
            npz_path,
            fluent_path,
            minimum_fluent_orthogonal_quality,
        )
    ...
    ansys_instructions = (
        ...
        "Static status: STATICALLY_VALIDATED_NOT_IMPORTED\n"
        + native_instruction
        + cgns_instruction
    ...
    try:
        ...
        publish_generation(
            ...
            upstream_artifacts=tuple(
                path
                for path in (params_path, context_step)
                if path is not None and path.is_file()
            ),
            case_name=str(report.get("case_name", outdir.name)),
        )
    finally:
        if stage.exists():
            shutil.rmtree(stage)
...
def run_body_fitted_case(inputs: BodyFittedCaseInputs) -> dict[str, Any]:
    ...
    try:
        ...
        publish_generation(
            ...
            operation="body-fitted-inlet",
            status="PASS",
            resolved_inputs=serialized_inputs,
            input_units={"geometry": "mm", "mesh": "m"},
            producer_files=(Path(__file__),),
            upstream_artifacts=tuple(
                path
                for path in (inputs.params, inputs.context_step)
                if path is not None and path.is_file()
            ),
            tool_versions={
...
def parse_args(argv: Sequence[str] | None = None) -> BodyFittedCaseInputs:
    parser = argparse.ArgumentParser(
        description="Build one body-fitted circular surface-inlet Hex8 mesh."
    )
    parser.add_argument(
        "--params",
        type=Path,
        default=Path("out/conical_journal/geometry/default/params.json"),
    )
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("out/conical_journal/meshing/body-fitted-inlet"),
    )
    parser.add_argument("--case-name", default="body-fitted-inlet")
    parser.add_argument("--topology", choices=("tensor-warp", "ogrid"), default="ogrid")
    parser.add_argument(
        "--geometry-mode", choices=("inscribed", "equal-area"), default="inscribed"
    )
    parser.add_argument("--q", type=int, default=16)
    parser.add_argument("--inner-layers", type=int, default=2)
    parser.add_argument("--outer-layers", type=int, default=4)
    parser.add_argument("--n-theta", type=int, default=256)
    parser.add_argument("--n-axial", type=int, default=96)
    parser.add_argument("--n-gap", type=int, default=12)
    parser.add_argument("--gap-inflation-ratio", type=float, default=1.0)
    parser.add_argument(
        "--quality-optimized-ogrid",
        action=argparse.BooleanOptionalAction,
        default=False,
    )
    parser.add_argument("--control-radius-factor", type=float, default=1.4)
    parser.add_argument("--control-square-blend", type=float, default=0.0)
    parser.add_argument("--central-corner-radius-factor", type=float, default=0.9)
    parser.add_argument("--smoothing-iterations", type=int, default=12)
    parser.add_argument("--smoothing-damping", type=float, default=0.25)
    parser.add_argument(
        "--smoothing-fixed-nodes",
        choices=("all-interfaces", "background-and-rim"),
        default="all-interfaces",
    )
    parser.add_argument("--minimum-fluent-orthogonal-quality", type=float)
    parser.add_argument("--openfoam", choices=("auto", "required", "skip"), default="skip")
    parser.add_argument("--ansys", choices=("auto", "required", "skip"), default="skip")
    parser.add_argument("--context-step", type=Path)
    values = parser.parse_args(argv)
    return BodyFittedCaseInputs(**vars(values))


def main(argv: Sequence[str] | None = None) -> int:
    try:
        report = run_body_fitted_case(parse_args(argv))
    except BodyFittedRunError as error:
