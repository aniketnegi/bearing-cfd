...
class SurfaceSmokeInputs:
    brep: Path = Path(
        "out/conical_journal/meshing/brep-preflight/film_zones_fragmented.brep"
    )
    preflight_report: Path = Path(
        "out/conical_journal/meshing/brep-preflight/preflight_report.json"
    )
    outdir: Path = Path("out/conical_journal/meshing/surface-smoke")
...
def _publish_smoke_success(
    ...
    publish_generation(
        ...
        producer_files=(Path(__file__),),
        upstream_artifacts=(Path(base_report["inputs"]["brep"]), preflight_report_path),
