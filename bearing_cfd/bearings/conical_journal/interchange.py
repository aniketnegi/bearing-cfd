...
def _readme_text(outdir: Path, case_dir: Path, fluent_passed: bool) -> str:
...
Automated audit when PyFluent and a CGNS/VKI license are available:
  bearing-cfd conical-journal export central-feed --case-dir {case_dir} --outdir {outdir} --fluent required --overwrite
...
def run_interchange(inputs: InterchangeInputs) -> dict[str, Any]:
    ...
    try:
        ...
        if inputs.fluent == "skip":
            ...
            from bearing_cfd.interchange.fluent_live import run_fluent_import_audit
        ...
        (stage / "README_OPEN_ME_FIRST.txt").write_text(
            _readme_text(inputs.outdir, inputs.case_dir, fluent_passed),
            encoding="utf-8",
        )
        publish_generation(
            stage,
            ...
            tool_versions={"gmsh": gmsh.__version__, "numpy": np.__version__},
        ...
        print(
            "4. Automated audit: bearing-cfd conical-journal export central-feed "
            f"--case-dir {inputs.case_dir} --outdir {inputs.outdir} "
