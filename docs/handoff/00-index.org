...
| Native Reynolds/JFO midsurface model through 2000 rpm | *unvalidated candidate* | The model converges and conserves its own discrete flow, but its 64.455% severe rupture region has not passed an external JFO benchmark or a three-grid study |
...
- *Unvalidated candidate* means numerical checks passed inside the current
  implementation, but external benchmark and grid-convergence evidence are
  still missing.  It must not be promoted to a validated physical result.
