...
import meshing.structured_hex_no_port as structured_hex
from meshing.structured_hex_no_port import (
    HEX_FACES,
    PHYSICAL_IDS,
    StructuredInputs,
    StructuredRunError,
...
DEFAULT_PARAMS = PROJECT_ROOT / "out" / "strict_default" / "params.json"
...
def test_all_true_scale_round_trips_and_visualization_markers(case: str) -> None:
    params_path = PROJECT_ROOT / "out" / case / "params.json"
    with tempfile.TemporaryDirectory(dir=PROJECT_ROOT / "out", prefix=f".structured-{case}-") as directory:
...
def test_failure_atomically_removes_staged_solver_outputs(monkeypatch: pytest.MonkeyPatch) -> None:
    with tempfile.TemporaryDirectory(dir=PROJECT_ROOT / "out", prefix=".structured-fail-") as directory:
        outdir = Path(directory) / "result"
        ...
        assert sorted(path.name for path in outdir.iterdir()) == ["failure_report.json"]
