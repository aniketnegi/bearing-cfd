...
from meshing import body_fitted_inlet_study
import meshing.structured_hex_body_fitted_inlet as body_fitted
from meshing import structured_hex_no_port as no_port
from meshing.structured_hex_surface_inlet import load_inlet_spec
...
def _geometry(case: str = "strict_default"):
    path = PROJECT_ROOT / "out" / case / "params.json"
...
def exported_tw16(
    ...
    report = body_fitted.run_body_fitted_case(
        body_fitted.BodyFittedCaseInputs(
            params=PROJECT_ROOT / "out" / "strict_default" / "params.json",
            outdir=outdir,
            case_name="TW16-I",
...
def test_quality_rejection_publishes_visual_only_previews(
    ...
    with pytest.raises(body_fitted.BodyFittedRunError):
        body_fitted.run_body_fitted_case(
            body_fitted.BodyFittedCaseInputs(
                params=(
                    PROJECT_ROOT / "out" / "strict_default" / "params.json"
                ),
                outdir=outdir,
