...
from meshing import structured_hex_body_fitted_inlet as body
from meshing.gmsh_brep_preflight import sha256_file
...
class StudyInputs:
    ...
    outdir: Path = Path("out/body_fitted_default")
    cases: tuple[str, ...] = ()
...
_INFLATION_STAIRCASE_DIRS = {
    "S390": _REPO_ROOT / "out/structured_surface_inlet_default/nGap_12"
}
...
def _render_study_images(
    ...
    script = _REPO_ROOT / "meshing/paraview_mesh_screenshot.py"
...
def parse_args(argv: Sequence[str] | None = None) -> StudyInputs:
    parser = argparse.ArgumentParser(
        description="Run the body-fitted surface-inlet comparison matrix."
    )
    parser.add_argument(
        "--preset",
        choices=tuple(PRESET_CASES),
        default=StudyInputs.preset,
    )
    parser.add_argument("--params", type=Path, default=StudyInputs.params)
    parser.add_argument("--outdir", type=Path, default=StudyInputs.outdir)
    parser.add_argument(
        "--cases",
        nargs="+",
        default=(),
        help="override the preset case list (space or comma separated)",
    )
    parser.add_argument(
...
def print_report(report: dict[str, Any]) -> None:
    print(
        f"{'case':<9} {'status':<6} {'topology':<12} {'faces':>8} {'Hex8':>9} "
        f"{'area.err':>10} {'minSICN':>10} {'maxNonOrth':>11}"
    )
    for row in report.get("cases", []):
        print(
            f"{row['case']:<9} {row['status']:<6} {row['topology']:<12} "
            f"{row['pressure_face_count']:8d} {row['hex8_count']:9d} "
            f"{row['area_relative_error']:10.3e} "
            f"{(row['minimum_minSICN'] or float('nan')):10.3g} "
            f"{(row['maximum_nonorthogonality_deg'] or float('nan')):11.3g}"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        report = run_study(parse_args(argv))
