...
class PreflightInputs:
    unsplit: Path = Path("out/strict_default/film_unsplit.brep")
    zones: Path = Path("out/strict_default/film_zones.brep")
    params: Path = Path("out/strict_default/params.json")
    outdir: Path = Path("out/gmsh_preflight")
    gui: bool = False
...
def atomic_replace_directory(stage: Path, outdir: Path) -> None:
    """Replace a generated-output directory without mixing old and new files."""
    backup = outdir.with_name(f".{outdir.name}-backup-{uuid.uuid4().hex}")
...
def _publish_preflight_success(
    ...
    report = {
        **base_report,
        ...
        "cad_params_note": (
            "The CAD report may be FAIL solely because strict STEP exchange failed; "
            "this preflight independently validates the native BREP."
        ),
